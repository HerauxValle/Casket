...
# obi - encrypted vault manager
#
# Usage:  obi <vault> <action> [options]
#         obi <action> [options]          (for global actions)
...
def find_img(name, path_override=None):
    if path_override:
        ...
        if not img.exists():
            die(f"vault '{name}' not found at {img}\n"
                f"    Hint: check the path or run 'obi list' to see all vaults.")
        return img, base
    ...
    die(f"vault '{name}' not found (searched cwd and 4 levels up)\n"
        f"    Hint: run 'obi list' to see all vaults, or cd to where it lives.")
...
def cmd_create(name, base, size, pw, strength):
    ...
    log(f"[obi] creating vault '{name}' ({size} MiB, strength={strength}) ...")
    ...
    try:
        ...
        log(f"    open it with:  obi {name} open")
...
def cmd_passwd(name, base, old_pw=None, new_pw=None, strength=None):
    ...
    if mnt.is_mount():   die(f"vault is open — close it first:  obi {name} close")
    if not old_pw:
    ...
    log(f"[obi] changing passphrase for '{name}' (strength={strength_label}) ...")
    meta_strip(img)
    try:
...
def cmd_2fa_on(name, base, pw):
    ...
    log(f"[obi] enabling 2FA on '{name}' ...")
    meta_strip(img)
...
def cmd_2fa_off(name, base, pw):
    ...
    log(f"[obi] disabling 2FA on '{name}' ...")
    meta_strip(img)
    new_meta = {}
...
def cmd_backup_create(name, base, snap_name):
    img, mnt, _ = resolve(base, name)
    if not img.exists(): die(f"vault '{name}' not found")
    if not mnt.is_mount(): die(f"vault is closed — open it first:  obi {name} open")
...
def cmd_backup_list(name, base):
    ...
    if not mnt.is_mount(): die(f"vault is closed — open it first:  obi {name} open")
    snap_root = _snap_root(mnt)
    snaps = sorted([s for s in snap_root.iterdir() if s.is_dir()],
                   key=lambda s: s.stat().st_mtime, reverse=True) if snap_root.exists() else []
    if not snaps:
        log(f"  no snapshots yet — create one with:  obi {name} backup create <n>")
        return
    log(f"  snapshots in '{name}' (newest first):")
    for s in snaps:
...
def cmd_backup_restore(name, base, snap_name):
    ...
    if not src.exists(): die(f"snapshot '{snap_name}' not found — run 'obi {name} backup list'")
    ...
    staging = mnt / f".obi-restore-{snap_name}"
...
def cmd_backup_delete(name, base, snap_name):
    ...
    if not snap.exists(): die(f"snapshot '{snap_name}' not found — run 'obi {name} backup list'")
...
def cmd_rename(vault, base, args):
    if not args:
        die("missing new name: obi <vault> rename <newname>")
    ...
    if subprocess.run(["mountpoint", "-q", str(mnt)]).returncode == 0:
        die(f"vault is open — close it first: obi {vault} close")
...
def cmd_resize(name, base, new_mb, pw):
    ...
    try:
        if shrink:
                ...
                if used_mb is not None:
                    ...
                    if new_mb < min_mb:
                        die(f"too small — vault contains ~{used_mb} MiB of data\n"
                            ...
                            f"    try:  obi {name} resize {min_mb}M")
                else:
            ...
            log(f"[obi] shrinking '{name}' {current_mb} -> {new_mb} MiB ...")
            ...
            log(f"[obi] resizing '{name}' {current_mb} -> {new_mb} MiB ...")
            run(["truncate", "-s", f"{new_mb}M", str(img)])
            cs_resize()
...
def cmd_close_all():
    log("[obi] closing all open vaults ...")
    # unmount everything from /proc/mounts that uses an imgvault mapper
    with open("/proc/mounts") as f:
        for line in f:
            parts = line.split()
            if len(parts) >= 2 and f"{MAPPER_PREFIX}_" in parts[0]:
                subprocess.run(["umount", "-l", parts[1]], stderr=subprocess.DEVNULL)
    # close all imgvault mappers
    try:
...
TYPICAL FIRST USE
  obi myvault create          # create a 1 GiB vault in current folder
  obi myvault open            # open it (prompts for passphrase)
  ...put files in myvault/...
  obi myvault close           # lock it again

Run 'obi help <action>' for details on any command, with examples.
"""

HELP_TOPICS = {
    "create": """
obi <vault> create [--size MiB] [--strength level] [--pass "..."]
...
EXAMPLES
  obi myvault create
  obi myvault create --size 4096 --strength hard
  obi myvault create --path ~/vaults
""",
    "open": """
obi <vault> open [--pass "..."] [--keyfile path]
...
EXAMPLES
  obi myvault open
  obi myvault open --keyfile /mnt/usb/my.key
""",
    "close": """
obi <vault> close
...
""",
    "passwd": """
obi <vault> passwd [--pass "..."] [--new-pass "..."] [--strength level]
...
EXAMPLE
  obi myvault passwd
  obi myvault passwd --pass "old" --new-pass "new" --no-log
  obi myvault passwd --strength hard
""",
    "2fa": """
obi <vault> 2fa on  [--pass "..."]
obi <vault> 2fa off [--pass "..."]
...
2FA means the vault needs BOTH a passphrase AND a keyfile to open.
  ...
  2fa off
    ...
    there first, then run 'obi <vault> 2fa off' again.
...
EXAMPLES
  obi myvault 2fa on
  obi myvault 2fa on --pass "mypassphrase" --no-log
  obi myvault 2fa off
""",
    "backup": """
obi <vault> backup create <name>   — create a readonly btrfs snapshot inside the vault
obi <vault> backup list            — list snapshots (newest first, with creation date)
obi <vault> backup restore <name>  — replace vault contents with a snapshot
obi <vault> backup delete <name>   — delete a snapshot

The vault must be open for all backup operations.
Snapshots live at /.obi-snapshots/<name> inside the vault.

restore asks for confirmation (skipped with --no-log).

EXAMPLES
  obi myvault backup create before-upgrade
  obi myvault backup list
  obi myvault backup restore before-upgrade
  obi myvault backup delete before-upgrade
""",
    "resize": """
obi <vault> resize <size>
...
Grow or shrink the vault. Size accepts any common unit (case-insensitive):
  ...
  Shrinking is destructive — obi will:
...
EXAMPLES
  obi myvault resize 2GiB
  obi myvault resize 20 GB
  obi myvault resize 512M
""",
    "delete": """
obi <vault> delete
...
('obi <vault> open') so the header is verified, then close and delete.
...
""",
    "encryption": """
obi <vault> encryption on  [--pass "..."]
obi <vault> encryption off [--pass "..."]
...
regardless of this setting — it controls how 'open' behaves.
  ...
  encryption off
    ...
    'obi <vault> open' (and toggle) will unlock without prompting.
  ...
  encryption on  (default)
    ...
    'obi <vault> open' requires your passphrase as normal.
...
EXAMPLES
  obi myvault encryption off
  obi myvault encryption on
  obi myvault encryption off --pass "mypass" --no-log
""",
    "list": """
obi list [--path dir]
...
def main():
    ...
    if action == "create":
        ...
        if not extra or extra[0] not in ("on", "off"):
            die("usage: obi <vault> 2fa on|off\n    Run 'obi help 2fa' for details.")
        _, base = find_img(vault, opt_path)
        pw = get_pw(opt_pass)
        if extra[0] == "on":
        ...
        if sub == "create":
            if len(extra) < 2:
                die("usage: obi <vault> backup create <name>\n    Example:  obi myvault backup create before-upgrade")
            _, base = find_img(vault, opt_path)
            ...
            if len(extra) < 2:
                die("usage: obi <vault> backup restore <name>\n    Example:  obi myvault backup restore before-upgrade")
            _, base = find_img(vault, opt_path)
            cmd_backup_restore(vault, base, extra[1])
        elif sub == "delete":
            if len(extra) < 2:
                die("usage: obi <vault> backup delete <n>\n    Example:  obi myvault backup delete old-snap")
            _, base = find_img(vault, opt_path)
            cmd_backup_delete(vault, base, extra[1])
        else:
            die("usage: obi <vault> backup create|list|restore|delete\n    Run 'obi help backup' for details.")
        ...
        if not extra:
            die("usage: obi <vault> resize <size>\n    Examples:  obi myvault resize 2048  |  obi myvault resize 20G  |  obi myvault resize 500MiB")
        ...
        log(f"    run 'obi help' to see all commands")
        sys.exit(1)
